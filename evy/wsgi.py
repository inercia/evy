...
class HttpProtocol(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def handle_one_response (self):
        ...
        def write (data, _writelines = wfile.writelines):
            ...
            try:
                ...
                self.server.log_message(
                    "Encountered non-ascii unicode while attempting to write wsgi response: %r" % [x
                                                                                                   for
                                                                                                   x
                                                                                                   in
                                                                                                   towrite
        ...
        try:
            ...
            if hasattr(result, 'close'):
                result.close()
            if (self.environ['evy.input'].chunked_input or
                self.environ['evy.input'].position\
                < self.environ['evy.input'].content_length):
                ## Read and discard body if there was no pending 100-continue
                if not self.environ['evy.input'].wfile:
                    while self.environ['evy.input'].read(MINIMUM_CHUNK_SIZE):
                        pass
            finish = time.time()

            for hook, args, kwargs in self.environ['evy.posthooks']:
    ...
    def get_environ (self):
        ...
        env['wsgi.input'] = env['evy.input'] = Input(
