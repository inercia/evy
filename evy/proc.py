...
called, an :class:`~evy.coros.Event` or a :class:`~evy.coros.queue`
...
There are also :meth:`~evy.proc.Source.link_value` and
:func:`link_exception` methods that only deliver a return value and an
unhandled exception respectively (plain :meth:`~evy.proc.Source.link`
...
must fail as well; :meth:`~evy.proc.Source.link_exception` is useful here:
...
"""
import sys
from evy import api, coros, hubs
...
def killall (procs, *throw_args, **kwargs):
    ...
    if kwargs:
        raise TypeError(
            'Invalid keyword argument for proc.killall(): %s' % ', '.join(kwargs.keys()))
    for g in procs:
        if not g.dead:
...
class Source(object):
    ...
    def _start_send (self):
        hubs.get_hub().schedule_call_global(0, self._do_send, self._value_links.items(),
                                            self._value_links)
    ...
    def _start_send_exception (self):
        hubs.get_hub().schedule_call_global(0, self._do_send, self._exception_links.items(),
                                            self._exception_links)
    ...
    def wait (self, timeout = None, *throw_args):
        ...
        *timeout* has expired. Return the argument of :meth:`send` or raise the
        ...
        :func:`~evy.api.timeout` treats them.
        """
        if self.value is not _NOT_USED:
            if self._exc is None:
                return self.value
