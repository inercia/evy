...
from evy import semaphore as semaphoremod
...
def execute (func, *args, **kw):
    ...
    This has the same api as the :meth:`evy.coros.CoroutinePool.execute`
    ...
    ('foo', 1)
    """
    warnings.warn("Coros.execute is deprecated.  Please use evy.spawn "
...
def CoroutinePool (*args, **kwargs):
    warnings.warn("CoroutinePool is deprecated.  Please use "
                  "evy.GreenPool instead.", DeprecationWarning, stacklevel = 2)
    from evy.pool import Pool
    ...
    def wait (self):
        if self.items:
            ...
            if exc is None:
                ...
                evy.getcurrent().throw(*exc)
        else:
            self._waiters.add(evy.getcurrent())
            try:
                ...
                self._waiters.discard(evy.getcurrent())
...
class Channel(object):
    ...
    def send (self, result = None, exc = None):
        if exc is not None and not isinstance(exc, tuple):
            exc = (exc, )
        if evy.getcurrent() is hubs.get_hub().greenlet:
            self.items.append((result, exc))
            if self._waiters:
            ...
            if len(self.items) > self.max_size:
                self._senders.add(evy.getcurrent())
                try:
                    hubs.get_hub().switch()
                finally:
                    self._senders.discard(evy.getcurrent())
...
class Actor(object):
    ...
    def __init__ (self, concurrency = 1):
        ...
        warnings.warn("We're phasing out the Actor class, so as to get rid of"
                      ...
                      "evydev@lists.secondlife.com, and we'll come up with a "
        ...
        self._killer = evy.spawn(self.run_forever)
    ...
    def received (self, message):
        ...
        >>> evy.kill(a._killer)   # test cleanup
        """
